...
to this repository only via a second, narrowly-scoped PAT. This threat model is updated below
(new asset, new trust-boundary row, new STRIDE row) to cover it; every other finding in this
...
| Metadata-write GitHub PAT (fine-grained, `contents: write` on this repo only, per ADR-0004) | **Secret, moderate value** | Scoped to a single non-sensitive file in one repository — cannot read or write any portfolio repo, and cannot write anything in this repo other than the metadata file's intended content path. Compromise's worst case is an unwanted or vandalized commit to a non-sensitive metadata file, not data exposure. |
...
| Backend API → GitHub (this repo only, ADR-0004) | Metadata file commit, second PAT in Authorization header | Bearer-token-authenticated dashboard request triggers the commit; GitHub PAT authenticates the commit itself | Scoped to `contents: write`, `metadata: read`, this repository only (ADR-0004) — no access to any portfolio repo | TLS | Backend must validate the edit is a well-formed metadata entry before committing (same tolerant-parse contract as reads, architecture.md §2) — malformed input must be rejected before it reaches GitHub, not merely tolerated on next read | GitHub's own commit history (this repo) |
...
| **Elevation of privilege** — in-app metadata-write path scope escape (ADR-0004) | Backend API → GitHub (metadata-write PAT) | An attacker who steals the bearer token (see the row above) can now also trigger real commits to this repo's metadata file, not just read data; or a bug in the API route lets the write target a path/repo beyond the intended metadata file | **Moderate** (new capability; was **None** before ADR-0004 since no write path existed) | The metadata-write PAT is scoped to `contents: write` on this repository only — it cannot reach any portfolio repo regardless of application-layer bugs. The API route must independently constrain the write to the one intended file path (PAT scope alone does not enforce a sub-path restriction within a repo) and must validate the payload shape before committing. | Bearer-token theft (already an accepted High-severity risk above) now also grants a limited git-commit capability on this one non-sensitive file, not just read access — a real increase in what a stolen token can do, though still bounded to one file in one low-sensitivity repo. **Accepted**, owner Josh, given the token-theft risk was already accepted and the added capability's blast radius is a single non-sensitive metadata file. |
